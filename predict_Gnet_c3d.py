...
class Predict(object):
    ...
    def build_sampler(self, layers, tparams, options, use_noise, trng):
        ...
        c3d0 = T.matrix('c3d_sampler', dtype='float32')
        c3d_mask = T.vector('c3d_mask', dtype='float32')
        ...
        c3d_ = c3d0
        counts_c3d = c3d_mask.sum(-1)
        c3d_mean = c3d_.sum(0)/counts_c3d
        c3d_max = c3d_.max(0)
        c3d_ = c3d0.dimshuffle('x',0,1)
        c3d_input = c3d_mean
        ...
        f_init = theano.function([ctx0, ctx_mask], [ctx0]+tu_init_state+tu_init_memory+
                         ...
                         profile=False)
        '''
        f_init = theano.function([ctx0, ctx_mask, c3d0, c3d_mask], [ctx0]+tu_init_state+tu_init_memory+
                                 mu_init_state+mu_init_memory+
                                 z_init_state+
                                 mu_p_init, name='f_init',
                                 on_unused_input='ignore',
        ...
        if options['att_fun'] == None:        
            mu_gru = layers.get_layer('lstm_cond')[1](options, tparams, tu_gru[0],
                                                   mask=None, context=lstm_cond_input,
                                                   one_step=True,
        ...
        f_next = theano.function([x, ctx0, ctx_mask]+
                         ...
                         on_unused_input='ignore')
        '''
        f_next = theano.function([x, ctx0, ctx_mask, c3d0, c3d_mask]+
                                 tu_init_state+tu_init_memory+
                                 mu_init_state+mu_init_memory+
                                 z_init+
                                 mu_p_init,
    ...
    def gen_sample(self, tparams, f_init, f_next, ctx0, ctx_mask, c3d, c3d_mask,
        ...
        rval = f_init(ctx0, ctx_mask,c3d, c3d_mask)
        ctx0 = rval[0]
        ...
        for ii in xrange(maxlen):
            ...
            rval = f_next(*([next_w, ctx0, ctx_mask, c3d, c3d_mask] +
    ...
    def sample_execute(self, engine, options, tparams, f_init, f_next, x, ctx, mask_ctx, c3d, c3d_mask, trng):
        stochastic = False
        for jj in xrange(np.minimum(10, x.shape[1])):
            sample, score,  _,_,_ = self.gen_sample(tparams, f_init, f_next, ctx[jj], mask_ctx[jj], c3d[jj], c3d_mask[jj],
