...
def generate_sample_gpu_single_process(
    ...
    def sample(whichset):
        samples = []
        ctxs, ctx_masks, c3ds, c3d_masks = engine.prepare_data_for_blue(whichset)
        for i, ctx, ctx_mask,c3d,c3d_mask in zip(range(len(ctxs)), ctxs, ctx_masks,c3ds,c3d_masks):
            print 'sampling %d/%d'%(i,len(ctxs))
            sample, score,  _,_,_ = model.gen_sample(None, f_init, f_next,
                                                   ctx, ctx_mask, c3d, c3d_mask ,None,
                                                   beam, maxlen=MAXLEN)
