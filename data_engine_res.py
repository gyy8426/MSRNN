...
class Movie2Caption(object):
    ...
    def __init__(self, model_type, signature, video_feature,
                 ...
                 n_frames=None, outof=None, options = None,
                 ):
    ...
    def pad_frames_mean(self, frames, limit, jpegs):
        # pad frames with 0, compatible with both conv and fully connected layers
        last_frame = frames[-1]
        mean_frame = np.mean(frames,axis=0)
        if jpegs:
            frames_padded = frames + [mean_frame]*(limit-len(frames))
        else:
            padding = np.asarray([mean_frame ]*(limit-len(frames)))
            frames_padded = np.concatenate([frames, padding], axis=0)
        return frames_padded    
    ...
    def load_data(self):
        ...
        if self.options is not None :
            if self.options['word2vec'] :
                
                if self.signature == 'msr-vtt':
                    self.word_ix = utils.load_pkl(dataset_path + 'word2vec/worddict_large.pkl')
                    self.Wemb = utils.load_pkl(dataset_path + 'word2vec/Wemb_large.pkl')
                elif self.signature == 'youtube2text':
                    self.word_ix = utils.load_pkl(dataset_path + 'word2vec/word2vec_worddict.pkl')
                    self.Wemb = utils.load_pkl(dataset_path + 'word2vec/Wemb.pkl')
        self.ix_word = dict()
